...
// 创建独立的StepHandlerRegistry实例并传递元数据
const registry = new StepHandlerRegistry();
registry.registerMetadata("mathOperation", mathOpMetadata);
...
stepsInfo.forEach(step => {
  ...
  console.log("\n输入Schema:");
  console.log(JSON.stringify(step.inputSchema, null, 2));

  console.log("\n输出Schema:");
  console.log(JSON.stringify(step.outputSchema, null, 2));
});
